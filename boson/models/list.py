...
class List(object):
    ...
    def create(self):
        ...
        DB().engine.execute(text(f"CREATE VIEW {self.view_name} AS SELECT * FROM {self.table_name}"))
    ...
    def _columns(self):
        ...
        columns.insert(0,Column('parent_id', Integer, ForeignKey(self.table_name + '.' + self._primary_key_name()), nullable=True))
        return columns
